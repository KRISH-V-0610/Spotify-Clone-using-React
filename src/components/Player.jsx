...
const Player = () => {
  ...
  const {track,seekBar,seekBg,play,pause,playStatus,time,prev,next,seekSong}=useContext(PlayerContext)
  ...
  return (
      ...
      <div className='flex flex-col items-center gap-1 m-auto'>
        ...
        <div className=' lg:ml-20 flex items-center gap-5'>
          <p>{time.currentTime.minute}:{time.currentTime.second}</p>
          <div onClick={seekSong}  ref={seekBg}className='w-[60vw] max-w-[500px] bg-gray-300 rounded-full cursor-pointer'>
            <hr ref={seekBar} className='h-[6px]  border-none w-0 bg-green-500 rounded-full' />
          </div>
